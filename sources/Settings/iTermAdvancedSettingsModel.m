...
DEFINE_BOOL(anchorVerticalTabsAtBottom, NO, SECTION_TABS @"Anchor tabs to the bottom of a left/right (vertical) tab bar?\nWhen enabled, the tab stack starts at the bottom edge and grows upward, leaving the empty space above the tabs. A scrollable side bar that overflows stays scrolled to the bottom (newest tab visible) until you scroll up; a non-scrollable one that overflows keeps the usual top-anchored layout with the overflow menu. Applies to open windows immediately.");
