...
DEFINE_BOOL(anchorVerticalTabsAtBottom, NO, SECTION_TABS @"Anchor tabs to the bottom of a left/right (vertical) tab bar?\nWhen enabled, the tab stack starts at the bottom edge and grows upward, leaving the empty space above the tabs. Has no effect while the tabs do not fit (overflow and scrolling keep the top-anchored layout). Applies to open windows immediately. (homelab fork)");
